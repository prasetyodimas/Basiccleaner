...
if ($act=='delete_pemesanan') {
	$transaksi_pesan = "DELETE ts, dts FROM transaksi_shoes ts INNER JOIN detail_transaksi_shoes dts 
	ON ts.id_transaksi_shoes=dts.id_transaksi_shoes WHERE dts.id_transaksi_shoes='$_GET[id]' AND ts.id_transaksi_shoes='$_GET[id]'"; 
	$action_delete = mysqli_query($con,$transaksi_pesan);
	/*echo $transaksi_pesan;
	exit();*/
	if ($action_delete) {
